...
@mens_stores.each {|u| puts "#{u.name} Store is generating #{u.annual_revenue} in revenue."} 

@womens_stores = Store.where(["womens_apparel = ? and annual_revenue > ?", "true", "1000000"])

@womens_stores.each {|w| puts "#{w.name} Store is generating #{w.annual_revenue} in revenue."}
